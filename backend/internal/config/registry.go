...
var Registry = slices.Concat(
	...
	group("Error tracking (Sentry)", "sentry",
		...
		floatKey("GFTP_SENTRY_SAMPLE_RATE", func(c *Config) *float64 { return &c.SentrySampleRate },
			...
			doc("Performance-trace sample rate between 0 and 1. 0 disables tracing; errors are unaffected.")),
		floatKey("GFTP_SENTRY_ERROR_SAMPLE_RATE", func(c *Config) *float64 { return &c.SentryErrorSampleRate },
			floatRange(0, 1), samples("0.25", "1.5"),
			doc("Fraction of error events sent, between 0 and 1. Lower it to cap quota burn when a remote server flaps.")),
		boolean("GFTP_SENTRY_CAPTURE_REMOTE_ERRORS", func(c *Config) *bool { return &c.SentryCaptureRemoteErrors },
			doc("Also report faults on the far side of the connection (unreachable host, dropped connection, timeout, TLS, remote disk full). Off by default: those are end-user and remote-server conditions, already covered by the access log and Prometheus.")),
		boolean("GFTP_SENTRY_SEND_SESSION_CONTEXT", func(c *Config) *bool { return &c.SentrySendSessionContext },
			doc("Attach the FTP username, remote host, and session prefix to events. Off by default: it sends end-customer identifiers to a third party.")),
	),
